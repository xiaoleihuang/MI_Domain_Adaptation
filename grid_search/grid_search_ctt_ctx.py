...
class SelfLogger(object):
    def __init__(self):
        ...
        self.log = open("./log/grid_ctt_ctx.log", "a")
            ...
            for dp_rate in dropout_rates:
                    for rnn_ac_func in rnn_ac_funcs:
                        for ac_func in ac_funcs:
                            ...
                            l_att_contxt_do = Dropout(dp_rate)(l_lstm_contxt)
                            ...
                            l_att_do = Dropout(dp_rate)(l_lstm)
                            ...
                            hist = test_model2.fit([context_train, x_train], code_train,
                                                epochs=epoch_nums,
                                                batch_size=64,
                                                validation_data=([context_valid, x_valid], code_valid),
                                                class_weight='auto')
...
best_model.save('../model/best_model_ctt_ctx.model')
print('------------------------------Testing-----------------------------------')
# multi label evaluation
y_pred = best_model.predict([context_test, x_test]);print()
...
pickle.dump(best_params, open('../params/best_params_ctt_ctx.pkl', 'wb'))
...
class SelfLogger(object):
    def __init__(self):
        ...
        self.log = open("./log/grid_ctt_ctx_codes.log", "a")
...
context_train = processed_contexts[train_indices]
context_valid = processed_contexts[valid_indices]
context_test = processed_contexts[test_indices]
labels_train = processed_labels[train_indices]
labels_valid = processed_labels[valid_indices]
labels_test = processed_labels[test_indices]
...
for w2v_path in word2vec_paths:
    ...
    embedding_matrix_labels = np.zeros((len(labels_tokenizer.word_index) + 1, 200))
    ...
    for label_tmp, i in labels_tokenizer.word_index.items():
        embeddings_vector_label = label2vec_model.get(label_tmp)
        # if word is found in the model, will be zeros
        if embeddings_vector_label is not None:
            embedding_matrix_labels[i] = embeddings_vector_label

    # balance training data or not
    # for if_bal in if_balance:
        # x_train = processed_dataset[train_indices]
        # context_train = processed_contexts[train_indices]
        # labels_train = processed_labels[train_indices]
        #
        ...
        # # balance data or not
        # if if_bal:
        #     context_train, _ = over_sampler.fit_sample(context_train, data_whole['codes'][train_indices])
        #     labels_train, _ = over_sampler.fit_sample(labels_train, data_whole['codes'][train_indices])
        #     x_train, _ = over_sampler.fit_sample(x_train, data_whole['codes'][train_indices])
        #     code_train, _ = over_sampler.fit_sample(code_train, data_whole['codes'][train_indices])
            ...
            for dp_rate in dropout_rates:
                for rnn_ac_func in rnn_ac_funcs:
                    for ac_func in ac_funcs:
                        ...
                        embedding_layer_pretrained_labels = Embedding(
                            len(labels_tokenizer.word_index) + 1, 200,
                            weights=[embedding_matrix_labels], input_length=10, trainable=True,
                            name='labels_embd')
                        ...
                        l_lstm_contxt = Bidirectional(GRU(gru_num, recurrent_dropout=dp_rate, kernel_initializer="glorot_uniform", recurrent_activation=rnn_ac_func),
                                                      name='context_bilstm')(embedded_sequences_contxt)
                        l_att_contxt_do = Dropout(dp_rate, name='ctx_dp')(l_lstm_contxt)
                        ...
                        l_lstm = Bidirectional(GRU(gru_num, recurrent_dropout=dp_rate, kernel_initializer="glorot_uniform", recurrent_activation=rnn_ac_func),
                                               name='content_bilstm')(embedded_sequences)
                        l_att_do = Dropout(dp_rate, name='ctt_dp')(l_lstm)

                        label_input = Input(shape=(10,), dtype='int32', name='labels_input')
                        embedded_sequences_labels = embedding_layer_pretrained_labels(label_input)
                        l_lstm_labels = Bidirectional(GRU(gru_num, recurrent_dropout=dp_rate, kernel_initializer="glorot_uniform", recurrent_activation=rnn_ac_func),
                                                      name='labels_bilstm')(embedded_sequences_labels)
                        l_att_labels_do = Dropout(dp_rate, name='label_dp')(l_lstm_labels)

                        merged_vector = keras.layers.concatenate([l_att_contxt_do, l_att_do, l_att_labels_do], axis=-1)

                        l_att_sent_dense = Dense(dense_num, activation=ac_func, kernel_initializer="glorot_uniform", name='final_nn')(merged_vector)
                        l_att_sent_drop = Dropout(dp_rate, name='final_dp')(l_att_sent_dense)
                        predictions = Dense(3, activation='sigmoid', name='final_output')(l_att_sent_drop)

                        test_model2 = Model(inputs=[sent_input_contxt, sent_input, label_input],
                                            outputs=predictions)
                        ...
                        hist = test_model2.fit([context_train, x_train, labels_train], code_train,
                                            epochs=epoch_nums,
                                            batch_size=64,
                                            validation_data=([context_valid, x_valid, labels_valid], code_valid),
                                            class_weight='auto')
...
best_model.save('../model/best_model_ctt_ctx_codes.model')
print('------------------------------Testing-----------------------------------')
# multi label evaluation
y_pred = best_model.predict([context_test, x_test, labels_test]);print()
...
pickle.dump(best_params, open('../params/best_params_ctt_ctx_codes.pkl', 'wb'))
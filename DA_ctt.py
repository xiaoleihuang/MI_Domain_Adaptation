...
def da_lstm_topic(word_embed, da_embed):
    """
    ...
    """
    # inputs
    ctt_input = Input(shape=(max_seqs, ), dtype='int32', name='text_input')
    ...
    da_embed = Embedding(
        da_embed.shape[0], da_embed.shape[1],
        weights=[da_embed], input_length=max_seqs, trainable=False,
        name='da_embed'
    )(da_input)
    ...
    flip = flipGradientTF.GradientReversal(hp_lambda, name='flip')(da_lstm) # flip gradient
    dense_da = Dense(dense_num, activation=ac_func, name='dense_da')(flip)
    preds_da = Dense(num_time, activation='softmax', name='preds_da')(dense_da)

    # Merge
    merge_inputs = keras.layers.concatenate([ctt_lstm, da_lstm], axis=-1)
...
if not os.path.exists('./weights/w2v.npy'):
    ...
    w2v_path = './preprocessed_data/w2v_corpus/google.bin'  # './preprocessed_data/w2v_corpus/glove.42B.300d.txt'
...
for e in range(epoch_num):
    ...
    for i in range(steps):
        ...
        tmp = my_model.train_on_batch(
            [ctt_train_batch, ctt_lda_train_batch],
    ...
    y_valid_preds = my_model.predict([ctt_valid, ctt_lda_valid])
    ...
    print('Testing..............')
    y_pred = my_model.predict([ctt_test, ctt_lda_test]);print()
    print(len(y_pred))
    print(len(y_pred[0]))
    y_pred = y_pred[0]
